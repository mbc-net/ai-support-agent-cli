...
export function generateWin32WrapperScript(opts: {
  ...
  const lines = [
    ...
    ``,
    `rem Obtain the image if the required version does not exist locally.`,
    `rem docker-ensure-image pulls the published image when nothing is customised`,
    `rem and builds locally otherwise (or when the pull fails).`,
    `docker image inspect "%IMAGE_TAG%" >nul 2>&1`,
    `if errorlevel 1 (`,
    `  echo Docker image %IMAGE_TAG% not found - obtaining... 1>&2`,
    `  call ai-support-agent docker-ensure-image`,
    `  if errorlevel 1 (`,
    `    echo ERROR: docker-ensure-image failed - cannot start container 1>&2`,
    `    exit /b 1`,
    `  )`,
    `)`,
    ``,
    ...
    ...setLines,
    ``,
    `rem NOTE: this Windows wrapper does NOT pass --user, so the container runs as`,
    `rem root and can write bind-mounted host dirs regardless of host ownership. The`,
    `rem "docker auto-creates a MISSING mount source as root -> in-container user`,
    `rem can't write it (EACCES on ~/.codex)" failure is specific to the Linux`,
    `rem (--user uid:gid) wrapper and does not apply here.`,
    dockerRun,
    ``,
    `endlocal`,
  ]
